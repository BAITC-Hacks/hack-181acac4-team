# HackAlem AI — от бизнес-проблемы до предложения команды
...
| Часть решения | Что работает | Как проверить |
| --- | --- | --- |
| Оформление задачи | Свободный ввод, три вопроса, редактирование всех полей | Создать задачу в разделе «Бизнесу» |
| Содержательное применение AI | Вопросы по описанию, извлечение цитат, проверка соответствия полям | Пройти сценарий с выключенным деморежимом |
| Контроль бизнеса | Публикация только после подтверждения; правка отменяет подтверждение | Попробовать публикацию до и после подтверждения |
| Геймификация | Баллы 0–100, четыре уровня, разбивка и подсказка следующего шага | Дополнить карточку и повторно подтвердить |
| Общий каталог | Опубликованные карточки, сортировка по рейтингу, фильтры темы и уровня | Найти задачу, в том числе с низким рейтингом |
| Предложения команд | Идея, план, срок, необязательная ссылка на прототип | Выбрать профиль и отправить несколько предложений |
| Ручной выбор | Принять или отклонить каждое предложение независимо | Принять две команды: одна не вытесняет другую |
| Воспроизводимость | Docker Compose, синтетические данные, автоматические проверки | Выполнить команды ниже |
...
Заполните `OPENAI_API_KEY` в `.env`. Файл исключён из Git; ключ используется только backend. Без ключа доступен явно обозначенный деморежим.
...
Перед показом запустите сервисы и загрузите примеры. Для демонстрации AI выключите «Деморежим без OpenAI». Время ответа зависит от внешнего API; для показа без сети используйте деморежим и явно обозначьте его.

| Время | Действие | Что видно |
| --- | --- | --- |
| 0:00–1:00 | Ввести описание кондитерской и ответить на три вопроса | Вопросы связаны с бизнес-задачей, появляется карточка |
...
Переключатель включает локальную заглушку. Она сохраняет описание в контекст, задаёт три шаблонных вопроса о пользователях, материалах и успехе, переносит ответы в соответствующие поля. Известные варианты «не знаю» пропускаются. Промпт OpenAI в этом режиме не вызывается; динамического анализа и смысловой проверки нет.

Формат входа: описание, затем ответы с `question_id`. Выход: тот же `TaskDraft` / `TaskCard`, что и в основном режиме. API включает заглушку через `?mode=demo` на создании черновика и отправке ответов. Валидация обязательных ответов и ручное подтверждение действуют в обоих режимах.
...
| OpenAI не отвечает | Проверить ключ, доступ к модели и соединение; повторить запрос или включить деморежим |
